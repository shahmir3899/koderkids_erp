// ============================================
// EDIT ACTIVITY MODAL - Update Existing Activity
// ============================================

import React, { useState, useEffect } from 'react';
import axios from 'axios';
import { updateActivity, fetchLeads, fetchBDMs } from '../../api/services/crmService';
import { getUserData, getAuthHeaders } from '../../utils/authHelpers';
import { API_URL } from '../../utils/constants';
import { toast } from 'react-toastify';

export const EditActivityModal = ({ activity, onClose, onSuccess }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    scheduled_date: activity.scheduled_date
      ? new Date(activity.scheduled_date).toISOString().slice(0, 16)
      : '',
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto">
      <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
        ...
        <div className="relative inline-block w-full max-w-2xl px-4 pt-5 pb-4 overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:p-6">
          {/* Header */}
          <div className="mb-6">
            <h3 className="text-2xl font-bold text-gray-900">Edit Activity</h3>
            <p className="mt-1 text-sm text-gray-600">
              Update activity details for {activity.lead_name}
            </p>
          </div>
