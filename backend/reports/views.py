...
def generate_pdf(request):
    ...
    try:
        ...
        # Create PDF document
        doc = SimpleDocTemplate(pdf_response, pagesize=A4, rightMargin=inch, leftMargin=inch, topMargin=inch, bottomMargin=inch)
        elements = []
        ...
        for i in range(0, 4, 2):
            ...
            for j in range(2):
                ...
                if idx < len(image_slots) and image_slots[idx]:
                    try:
                        img_response = requests.get(image_slots[idx], timeout=5)
        ...
        print("Building PDF with elements:", len(elements))  # Debug log
        doc.build(elements)
        print("PDF built successfully")  # Debug log
        return pdf_response
