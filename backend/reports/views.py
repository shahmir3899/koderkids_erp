...
def fetch_image(url, timeout=15, max_size=(1200, 1200)):
    ...
    try:
        ...
        logger.debug(f"Attempting to fetch image from {url}")
        response = requests.get(url, headers=headers, timeout=timeout, allow_redirects=True)
        logger.info(f"Image fetch response - Status: {response.status_code}, Size: {len(response.content)} bytes")
        response.raise_for_status()
...
def student_report_data(request):
    ...
    try:
        ...
        if not student_id or not mode or (mode == 'month' and not month) or (mode == 'range' and not (start_date and end_date)):
            ...
            return Response({'message': 'Failed to fetch report data', 'error': 'Missing required parameters: student_id, mode, and either month or start_date/end_date are required'}, status=400)
        ...
        if mode == 'month':
            ...
            try:
                ...
                return Response({'message': 'Failed to fetch report data', 'error': 'Invalid date format. Use YYYY-MM-DD'}, status=400)
        ...
        logger.error(f"Unexpected error in student_report_data: {str(e)}", exc_info=True)
        return Response({"message": "Failed to fetch report data", "error": "An unexpected error occurred. Please try again later."}, status=500)
...
def generate_pdf(request):
    ...
    try:
        ...
        if not all([student_id, mode, school_id, student_class]):
            ...
            return Response({'message': 'Failed to generate PDF', 'error': 'Missing required parameters: student_id, mode, school_id, and student_class are required'}, status=400)
        ...
        if user.role == "Teacher":
            if int(school_id) not in user.assigned_schools.values_list("id", flat=True):
                logger.warning(f"Unauthorized access attempt by {user.username} to school {school_id}")
                return Response({"message": "Failed to generate PDF", "error": "Unauthorized access to this school"}, status=403)
        ...
        if mode == 'month':
            ...
            try:
                ...
                logger.warning(f"Invalid date format: start_date={start_date}, end_date={end_date}")
                return Response({'message': 'Failed to generate PDF', 'error': 'Invalid date format. Use YYYY-MM-DD'}, status=400)
        ...
        if "error" in supabase_response:
            logger.error(f"Supabase error fetching images for student {student_id}: {supabase_response['error']['message']}", exc_info=True)
            image_urls = []
        else:
            all_images = [
                supabase.storage.from_(settings.SUPABASE_BUCKET).create_signed_url(
                ...
                if idx < len(image_slots) and image_slots[idx]:
                    try:
                        ...
                        try:
                            ...
                            logger.error(f"Test render failed for image {idx+1}: {str(e)}", exc_info=True)
                            row.append(Paragraph(f"Image {idx+1} (Render error)", normal_style))
                            continue
                ...
                lessons_table = Table(
                    [['Date', 'Planned Topic', 'Achieved Topic']] + [
                        [
                            ...
                            Paragraph(
                                f"{lesson['achieved_topic']} " + 
                                ('<font color="green">âœ“</font>' if lesson['planned_topic'] == lesson['achieved_topic'] and lesson['achieved_topic'] != "N/A" else ''),
                                normal_style
                            )
                        ]
                        for lesson in lessons_data
