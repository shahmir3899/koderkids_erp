...
def fetch_image(url, timeout=15, max_size=(1200, 1200)):
    """
    Fetch image from URL with robust error handling and optional resizing.
    ...
    try:
        # Disable compression explicitly and add referer
        headers = {
        ...
        # Process image with PIL for resizing and format consistency
        try:
            ...
            img_format = 'JPEG' if content.startswith(b'\xff\xd8') else 'PNG'
            ...
            logger.error(f"PIL processing error: {str(pil_error)}")
            # Fallback to original content if PIL fails
...
def generate_pdf(request):
    ...
    try:
        ...
        attendance_data = {
            ...
            "percentage": (attendance_records.filter(status="Present").count() / total_days * 100) if total_days > 0 else 0
        }
        ...
        lessons_data = [
            {
                ...
                "achieved_topic": achieved_lessons.filter(session_date=lesson["session_date"]).first()["achieved_topic"] if achieved_lessons.filter(session_date=lesson["session_date"]).exists() else "N/A"
            }
...
# Note: The batch PDF generation function would follow similar patterns
