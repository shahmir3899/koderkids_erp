...
"""
Student Report Generator Module

This module provides endpoints for generating student reports in PDF format,
including attendance data, lesson progress, and student images.
"""
...
MAX_IMAGE_PIXELS = 50 * 10**6      # 50 megapixels
IMAGE_TIMEOUT = 15                 # seconds
SUPPORTED_FORMATS = {
    'jpeg': b'\xff\xd8',
    'jpg': b'\xff\xd8',
    'png': b'\x89PNG',
    'webp': b'RIFF'  # Simplified WEBP signature
}
...
def fetch_image(url, timeout=IMAGE_TIMEOUT, max_size=(1200, 1200)):
    """
    Fetch and process an image from a URL with robust error handling.
    
    Args:
        url (str): The image URL
        timeout (int): Request timeout in seconds
        max_size (tuple): Maximum dimensions for resizing
        
    Returns:
        BytesIO: Processed image data or None if failed
    """
    if not url:
    ...
    try:
        ...
        if not content.startswith(SUPPORTED_FORMATS[extension]):
            logger.warning(f"Invalid image signature for {extension}")
            return None
        ...
        try:
            ...
            if img.size[0] * img.size[1] > MAX_IMAGE_PIXELS:
                logger.warning("Image dimensions too large (possible decompression bomb)")
                return None
            ...
            # Strip metadata and save to buffer
            img.info = {}  # Remove EXIF data
            output_buffer = BytesIO()
            
            img_format = 'JPEG' if extension in ('jpeg', 'jpg') else extension.upper()
            img.save(output_buffer, format=img_format, quality=85)
            output_buffer.seek(0)

            # Validate final size
            if output_buffer.getbuffer().nbytes > MAX_IMAGE_SIZE:
                logger.warning("Processed image exceeds size limit")
                return None

            logger.debug(f"Successfully processed image: {img.size}")
            return output_buffer

        except Exception as pil_error:
            logger.error(f"Image processing error: {str(pil_error)}")
            return None
...
def student_report_data(request):
    """
    Fetch student report data in JSON format.
    
    Returns:
        Response: JSON containing student data, attendance, lessons, and images
    """
    user = request.user
    try:
        ...
        if not student_id or not mode or (mode == 'month' and not month) or \
           (mode == 'range' and not (start_date and end_date)):
            return Response({'error': 'Missing required parameters'}, status=400)
        ...
        attendance_data = {
            ...
            "not_marked": next((item['count'] for item in attendance_records if item['status'] == "N/A"), 0),
            "total_days": total_days,
            "percentage": round((attendance_data["present"] / total_days * 100), 2) if total_days > 0 else 0.0
        }
...
@permission_classes([IsAuthenticated])
def generate_pdf(request):
    """Generate a PDF report for a single student with enhanced image handling."""
    user = request.user
    try:
        # Parameter extraction and validation
        required_params = {
        ...
        if None in required_params.values():
            ...
            return Response({'error': f'Missing parameters: {", ".join(missing)}'}, status=400)

        mode = required_params['mode']
        if mode == 'month' and not request.GET.get('month'):
            return Response({'error': 'Month required for monthly reports'}, status=400)
        if mode == 'range' and not (request.GET.get('start_date') and request.GET.get('end_date')):
            return Response({'error': 'Date range required'}, status=400)

        # Authorization check
        if user.role == "Teacher":
            assigned_schools = user.assigned_schools.values_list("id", flat=True)
            if int(required_params['school_id']) not in assigned_schools:
                return Response({"error": "Unauthorized access"}, status=403)

        # Get student and date range
        student = Student.objects.filter(
            id=required_params['student_id'],
            school_id=required_params['school_id'],
            student_class=required_params['student_class']
        ).first()
        ...
        attendance_records = Attendance.objects.filter(
            student_id=required_params['student_id'],
            session_date__range=[start_date, end_date]
        ).values('status').annotate(count=Count('id'))
        ...
        # Fetch and process images
        folder_path = f"{required_params['student_id']}/"
        supabase_response = supabase.storage.from_(settings.SUPABASE_BUCKET).list(folder_path)
        
        if "error" in supabase_response:
            logger.error(f"Supabase error: {supabase_response['error']['message']}")
            image_urls = []
        else:
            prefix = month if mode == 'month' else start_date.strftime('%Y-%m')
            all_images = [
        ...
        if lessons_data:
            ...
            elements.append(Paragraph("No lessons found for the selected period.", normal_style))
        elements.append(Spacer(1, 6*mm))
        ...
        if image_urls:
                ...
                for j in range(2):
                    ...
                    if idx < len(image_slots) and image_slots[idx]:
                        try:
                            ...
                            img_data = fetch_image(img_url)
                            if img_data is None:
                                logger.warning(f"Image fetch failed for {img_url}")
                                row.append(Paragraph(f"Image {idx+1} (Unavailable)", normal_style))
                                continue
                            ...
                            target_width, target_height = 75*mm, 50*mm
                            with PILImage.open(img_data) as pil_img:
                                natural_size = pil_img.size
                            
                            img_ratio = natural_size[0] / natural_size[1] if natural_size[1] > 0 else 1
            ...
            if len(image_urls) > 4:
                elements.append(Paragraph(
                    f"Note: Showing 4 of {len(image_urls)} available images.", 
                    ParagraphStyle(
                        name='Small',
                        fontSize=8,
                        textColor=colors.gray,
...
def generate_pdf_batch(request):
    ...
    try:
        ...
        # Validate required parameters
        if not student_ids or not mode or (mode == 'month' and not month) or \
           (mode == 'range' and not (start_date and end_date)):
            return Response({'error': 'Missing required parameters'}, status=400)
        ...
        with zipfile.ZipFile(zip_buffer, 'w', compression=zipfile.ZIP_DEFLATED) as zip_file:
            for student_id in student_ids:
                ...
                # Generate PDF for each student (similar to single PDF generation)
